...
const wrapperJs = readFileSync(path.join(root, "livecam-wrapper.js"), "utf8");
...
let uiHtml = null;
const uiPath = path.join(path.dirname(path.resolve(outPath)), "livecam-ui.html");
try {
...
function uiPayloadJs(buf) {
	// Deflate keeps the embedded script small - Max's [js] handled ~340 KB
	// scripts in sibling devices but livecam's UI is larger, so don't tempt
	// any size ceiling. wrapper.js carries a matching ES5 inflater.
	...
	for (let i = 0; i < deflated.length; i += CHUNK) {
		chunks.push(JSON.stringify(deflated.subarray(i, i + CHUNK).toString("base64")));
	}
	return (
		"\n// ---- generated by build-amxd.mjs: embedded UI payload (deflate-raw) ----\n" +
		`var UI_PAYLOAD_BYTES = ${buf.length};\n` +
		`var UI_PAYLOAD_DEFLATE = 1;\n` +
		`var UI_PAYLOAD_B64 = [\n${chunks.join(",\n")}\n];\n`
	);
}
...
const files = [
	...
	{ type: "TEXT", name: "livecam-wrapper.js", data: Buffer.from(wrapperData, "utf8"), flag: 0 },
];
